...
from args import get_PATE_config
...
from solver_pate import Solver_PATE
...
def main(config):
    ...
    if config.mode == 'train':
        ...
        if not config.non_private:
            log = f"==> Note we will training with PATE, with sigma = {config.sigma} and ... " \
                  f"sigma_thresh = {config.sigma_thresh}"
            print(log)
            logging.info(log)
        else:
        ...
        solver = Solver_PATE(config, log=logging)

    elif config.mode == 'test':
        solver = Solver_PATE(config)
    else:
