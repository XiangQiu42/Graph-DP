...
def check_validity(dataset, dataset_root):
    all_smiles = None
    if dataset_root.endswith('.smi'):
        all_smiles = [line for line in open(dataset_root, 'r').readlines()]
    else:
...
def novelty_metric(dataset, dataset_root, orig_data_root):
    with open(orig_data_root, 'rb') as f:
        all_smiles = set(pickle.load(f))
    with open(dataset_root, 'rb') as f:
...
def count_edge_type(dataset, dataset_root, generated=True):
    if generated:
        filename = dataset_root
    else:
        filename = 'all_smiles_%s.pkl' % dataset

    all_smiles = load_smiles(dataset_root)
...
def count_atoms(dataset, dataset_root):
    all_smiles = load_smiles(dataset_root)
...
def check_uniqueness(dataset, dataset_root):
    all_smiles = load_smiles(dataset_root)
...
def shape_count(dataset, dataset_root, remove_print=False, all_smiles=None):
...
def check_cyclic(dataset, dataset_root, generated=True):
    if generated:
        all_smiles = load_smiles(dataset_root)
    else:
...
def check_sascorer(dataset, dataset_root):
    all_smiles = load_smiles(dataset_root)
    sa_sum = 0
    total = 0
...
def check_logp(dataset, dataset_root):
    all_smiles = load_smiles(dataset_root)
    logp_sum = 0
    total = 0
...
def check_qed(dataset, dataset_root):
    all_smiles = load_smiles(dataset_root)
    qed_sum = 0
    total = 0
...
def sssr_metric(dataset, dataset_root):
    all_smiles = None
    if dataset_root.endswith('.smi'):
        all_smiles = [line for line in open(dataset_root, 'r').readlines()]
    else:
