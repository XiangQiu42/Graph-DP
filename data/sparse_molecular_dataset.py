...
class SparseMolecularDataset:
    ...
    def load(self, filename, num_subsets=1):

        with open(filename, 'rb') as f:
            self.__dict__.update(pickle.load(f))

        for i in range(num_subsets):
            self.train_idx_list[i] = np.random.choice(self.train_idx_list[i], int(len(self.train_idx_list[i])),
                                                      replace=False)
        self.validation_idx = np.random.choice(self.validation_idx, int(len(self.validation_idx)),
                                               replace=False)
        self.test_idx = np.random.choice(self.test_idx, int(len(self.test_idx)), replace=False)

        self.train_count = len(self.train_idx_list[0])
        self.validation_count = len(self.validation_idx)
        self.test_count = len(self.test_idx)

        self.__len = self.train_count * num_subsets + self.validation_count + self.test_count
    ...
    def generate(self, filename, add_h=False, filters=lambda x: True, size=None, validation=0.1, test=0.1,
        ...
        self._generate_train_validation_test(validation, test, num_subset=num_subset)
    ...
    def _generate_train_validation_test(self, validation, test, num_subset=1):
        ...
        self.train_idx_list = []
        trainset_size = int(train / num_subset)
        for i in range(num_subset):
            start = i * trainset_size
            end = (i + 1) * trainset_size
            indices = self.all_idx[start:end]
            self.train_idx_list.append(indices)
        self.validation_idx = self.all_idx[train:train + validation]
        self.test_idx = self.all_idx[train + validation:]

        self.train_counter = []
        self.train_count = trainset_size
        for i in range(num_subset):
            self.train_counter.append(0)
        self.validation_counter = 0
        self.test_counter = 0
        self.validation_count = validation
        self.test_count = test

        self.log('Created train ({} items), validation ({} items) and test ({} items) sets, and split the train set '
                 'to {} subset, each has {} samples!'.format(train, validation, test, num_subset, trainset_size))

    def _next_batch(self, counter, count, idx, batch_size):
        if batch_size is not None:

            _index = []
            while batch_size - len(_index) > count:
                np.random.shuffle(idx)
                _index.extend(idx)

            # rem batch_size
            bs = batch_size - len(_index)
            if counter + bs >= count:
                counter = 0
                np.random.shuffle(idx)

            _index.extend(idx[counter:counter + bs])
    ...
    def next_train_batch(self, batch_size=None, subset_index=0):
        out = self._next_batch(counter=self.train_counter[subset_index], count=self.train_count,
                               idx=self.train_idx_list[subset_index], batch_size=batch_size)
        self.train_counter[subset_index] = out[0]
...
if __name__ == '__main__':
    ...
    # # GDB9 dataset
    data.generate('gdb9.sdf', filters=lambda x: x.GetNumAtoms() <= 9, num_subset=1)
    data.save('gdb9_9nodes_1.sparsedataset')

    # DP train set generation with subsampling rate = 1/1000
    data.generate('gdb9.sdf', filters=lambda x: x.GetNumAtoms() <= 9, num_subset=1000)
    data.save('gdb9_9nodes_1000.sparsedataset')

    # # QM9_5K subset dataset
    # data.generate('qm9_5k.smi', validation=0.026, test=0.026, filters=lambda x: x.GetNumAtoms() <= 9,
    #               num_subset=1)
    #
    # # DP train set generation with subsampling rate = 1/100
    # data.save('qm9_5k_1.sparsedataset')

    # Zinc dataset
    # Note the datasets can be download at https://github.com/drigoni/ComparisonsDGM
    # zinc is a datasets with about ~23 atoms
    # data.generate('250k_rndm_zinc_drugs_clean.smi', filters=lambda x: x.GetNumAtoms() <= 20, num_subset=1)
    #
    # # DP train set generation with subsampling rate = 1/100
    # data.save('zinc_1.sparsedataset')
